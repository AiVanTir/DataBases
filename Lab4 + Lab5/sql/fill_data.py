...
def create_events(cur, refs, characters):
    ...
    boss_by_map = {}
    for row in refs["boss_pool"]:
        boss_by_map.setdefault(row["map_id"], []).append(row["boss_id"])
    ...
    for i in range(EVENTS_COUNT):
        ...
        side1_result = 'W' if event_type != 'battleground' else random.choice(['W', 'L'])
        cur.execute(
            '''
        ...
        if event_type == "battleground":
            ...
            bosses = boss_by_map.get(event_map["map_id"], [])
            if bosses:
                for boss_id in random.sample(bosses, min(len(bosses), random.randint(1, 2))):
                    cur.execute(
                        '''
                        INSERT INTO "EventBoss" (event_id, boss_id, side_id, map_id)
                        VALUES (%s, %s, %s, %s)
                        ''',
                        (event_id, boss_id, side1_id, event_map["map_id"]),
                    )
